    ...
    <div class="container">
        ...
        @if($sproducts->count() > 0)            
            <div class="wrap-show-advance-info-box style-1 has-countdown">
                ...
                <div class="wrap-countdown mercado-countdown" data-expire="2022/11/29 12:00:00"></div>
                    ...
                    <div class="tab-contents">
                        @foreach($categories as $key => $category)
                            <div class="tab-content-item {{$key == 0 ? 'active' : ''}}" id="category_{{$category->id}}">
                                <div class="wrap-products slide-carousel owl-carousel style-nav-1 equal-container" data-items="5" data-loop="false" data-nav="true" data-dots="false" data-responsive='{"0":{"items":"1"},"480":{"items":"2"},"768":{"items":"3"},"992":{"items":"4"},"1200":{"items":"5"}}' >
                                    @php
                                        $cat_products = DB::table('products')->where('category_id', $category->id)->get()->take($no_of_products);
                                    @endphp
                                    
                                        @foreach($cat_products as $cat_product)
                                            <div class="product product-style-2 equal-elem ">
                                                <div class="product-thumnail">
                                                    <a href="{{route('product.details', ['slug' => $cat_product->slug])}}" title="{{$cat_product->name}}">
                                                        <figure><img src="{{ asset('assets/images/products')}}/{{$cat_product->image }}" width="800" height="800" alt="{{$cat_product->name}}"></figure>
                                                    </a>
                                                </div>
                                                <div class="product-info">
                                                    <a href="{{route('product.details', ['slug' => $cat_product->slug])}}" class="product-name"><span>{{$cat_product->name}}</span></a>
                                                    <div class="wrap-price"><span class="product-price">{{$cat_product->regular_price}}</span></div>
                                                </div>
                                            </div>
                                        @endforeach
                                </div>
